...
class Employees(Base):
    ...
    def post(self, request):
        ...
        enterprise_id = self.get_enterprise_id(request.user.id)
        signup_user = Authentication.sign_up(self, name=name, email=email, password=password, type_account='employee', company_id=enterprise_id)
...
class EmployeeDetail(Base):
    ...
    def put(self, request, employee_id):
        ...
        if email != employee.user.email and User.objects.filter(email=email).exists():
            raise APIException("Já existe um usuário com este e-mail.", code="email_already_use")
        
        User.objects.filter(id=employee.user.id).update(name=name, email=email)

        UserGroup.objects.filter(user_id=employee.user.id).delete()
        ...
        if groups:
            ...
            for group_id in groups:
                ...
                UserGroup.objects.create(user_id=employee.user.id, group_id=group_id)
    ...
    def delete(self, request, employee_id):
        ...
        if check_if_owner:
            raise APIException("Não é possível excluir o proprietário da empresa.", code="cannot_delete_owner")
